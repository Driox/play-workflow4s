...
class Fail(val message: String, val cause: Option[Either[Throwable, Fail]] = None) {
  ...
  def withEx(fail: Fail): Fail = {
    val new_cause = cause
      .map {
        case Left(t)  => Right(fail.withEx(t))
        case Right(f) => Right(f.withEx(fail))
      }
      .orElse(Some(Right(fail)))
...
object Fail {
  ...
  def apply(jsError: scala.collection.Seq[(JsPath, scala.collection.Seq[JsonValidationError])]): Fail = {
